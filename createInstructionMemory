...
int main(int argc, const char * argv[]) {
    ...
    while (fgets(line, 100, assembly_program) != NULL){
        ...
        while(token != NULL){
            instructionMemory[j][i]= strdup(token);
            token = strtok(NULL, delimiters);
            i++;
        }
        //still working on verifyInstruction.
        assert(verifyInstruction(instructionMemory[j]));
        j++;
    }
    for(i=0;i<j;i++){printf("%s %s %s %s %s\n", instructionMemory[i][0], instructionMemory[i][1], instructionMemory[i][2], instructionMemory[i][3], instructionMemory[i][4]);}
